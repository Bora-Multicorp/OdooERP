...
class SaleOrderLine(models.Model):
    ...
    @api.onchange("price_unit", "tax_id", "order_id.currency_id")
    def _onchange_compute_price_incl(self):
        if self.ks_editing_from_incl:
            # This fired as a cascade from _onchange_price_unit_incl updating price_unit.
            # Reset the flag and skip — price_unit_incl already holds the user's value.
    ...
    def _onchange_price_unit_incl(self):
        taxes = self.tax_id
        currency = self.currency_id
        # Guard: if incl already matches what excl+tax gives, this is a cascade — skip.
        if taxes:
            ...
            return
        # Flag tells _onchange_compute_price_incl to skip when it fires as a
        # cascade after price_unit is updated below.
    ...
    def write(self, vals):
        ...
        if not self.env.context.get('_skip_price_incl_sync'):
            if ('price_unit' in vals or 'tax_id' in vals) and 'price_unit_incl' not in vals:
                for record in self:
                    record._sync_price_incl()
        return res
