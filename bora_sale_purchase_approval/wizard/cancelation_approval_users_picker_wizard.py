...
class SOCancelationApprovalUsersPicker(models.TransientModel):
    ...
    sale_id = fields.Many2one('sale.order', string="Cancelation for Sale Order Cancelation")

    approver1_users = fields.Many2many(
        comodel_name='sale.order.cancellation.approvers',
        relation='picker_wizard_approver1_rel_cancel',   # custom relation table
    ...
    approver2_users = fields.Many2many(
        comodel_name='sale.order.cancellation.approvers',
        relation='picker_wizard_approver2_rel_cancel',   # DIFFERENT relation table
        column1='wizard_id',
        column2='approver_id',
        string="Approver 2"
    )
    ...
    def add_users_for_approval(self):
        ...
        approvers = self.env['sale.order.cancellation.approvers'].browse(id_list)
        self.sale_id.assign_cancel_users(approvers)
    ...
    def default_get(self, fields):
        ...
        Approver = self.env['sale.order.cancellation.approvers']
