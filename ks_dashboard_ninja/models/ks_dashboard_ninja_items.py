...
import dateutil
import pandas as pd
from odoo.tools import sql, SQL, html2plaintext
import pytz
...
class KsDashboardNinjaItems(models.Model):
    ...
    @api.model
    def dn_models_in_use(self):
        """
        Return a tuple of model technical names referenced by any dashboard item or item source.
        Cached to avoid repeated queries; cache is cleared on item/source changes.
        """
        if DN_MODELS_IN_USE:
            return DN_MODELS_IN_USE

        try:
            items = self.sudo().search_read([], ['ks_model_name', 'ks_model_name_2'])
            DN_MODELS_IN_USE.update(rec['ks_model_name'] for rec in items if rec.get('ks_model_name'))
            DN_MODELS_IN_USE.update(rec['ks_model_name_2'] for rec in items if rec.get('ks_model_name_2'))
        except Exception:
            pass

        return tuple(sorted(DN_MODELS_IN_USE))
    ...
    ks_sort_by_field = fields.Many2one('ir.model.fields',
                                       ...
                                       string="Sort By Field", help='Select the desired sorting preference. ', ondelete='set null')
    ks_sort_by_order = fields.Selection([('ASC', 'Ascending'), ('DESC', 'Descending')], string="Sort Order", help='Select the order of the sorting.')
    ...
    ks_kpi_type = fields.Selection([ ('layout_1', 'KPI With Target'), ('layout_2', 'Data Comparison')], string="Kpi Layout", default="layout_1")
    ...
    ks_list_view_fields = fields.Many2many('ir.model.fields', 'ks_dn_list_field_rel', 'list_field_id', 'field_id',
                                           domain="[('model_id','=',ks_model_id),('ttype','!=','one2many'),"
                                                  "('ttype','!=','many2many'),('ttype','!=','binary'), ('ttype','!=','properties')]",
    ...
    ks_map_chart_relation_groupby = fields.Many2one('ir.model.fields', domain="[('model_id','=',ks_model_id),('name','!=','id'),('name','!=','sequence'),"
                                                       "('store','=',True),('ttype','!=','binary'),"
    ...
    data_source = fields.Selection([('odoo', 'Odoo'), ('excel', 'Excel'), ('csv', 'CSV')], string="Data Source",default='odoo')
    ks_ai_analysis = fields.Char(string='AI Analysis')
    ks_kpi_previous_period_data = fields.Float(compute='_ks_get_previous_period_data', compute_sudo=False)
        ...
        try:
            ...
            for item in data:
                ...
                if item["chart_type"].lower() in ['bar', 'line', 'pie', 'area', 'donut', 'polar', 'horizontalbar']:
                    ...
                    pass
            # FIXME: Why so many if , else - should be improved, logic is bad should be rewritten
    ...
    def write(self, vals):
        ...
        res = super(KsDashboardNinjaItems, self).write(vals)

        if vals.get('ks_model_id') or vals.get('ks_model_id_2'):
            global DN_MODELS_IN_USE
            DN_MODELS_IN_USE.clear()
        return res
    ...
    def _ks_onchange_ks_data_calculation_type(self):
        for rec in self:
            if rec.ks_data_calculation_type == 'query':
                ...
                # fixme: find another solution for it
    ...
    def create(self, values):
        ...
        records = super(KsDashboardNinjaItems, self).create(values)
        global DN_MODELS_IN_USE
        DN_MODELS_IN_USE.clear()
        return records
    ...
    @api.depends('ks_record_count_type', 'ks_model_id', 'ks_domain', 'ks_record_field', 'ks_date_filter_field',
                 'ks_item_end_date', 'ks_item_start_date', 'ks_compare_period', 'ks_year_period',
                 'ks_dashboard_item_type', 'ks_domain_extension', 'ks_data_format')
    def _ks_get_previous_period_data(self):
        for rec in self:
            rec.ks_kpi_previous_period_data = rec.ks_get_previous_period_data()

    def unlink(self):
        channel = self.env['discuss.channel'].search([('ks_dashboard_item_id', 'in', self.ids)])
        if channel:
            channel.unlink()
        res = super(KsDashboardNinjaItems, self).unlink()
        global DN_MODELS_IN_USE
        DN_MODELS_IN_USE.clear()
        return res
                ...
                if selected_end_date and not selected_start_date:
                    ...
                    if selected_end_date and selected_start_date:
                        if isinstance(selected_start_date, str):
                            selected_start_date = parser.parse(selected_start_date)
                        elif not isinstance(selected_start_date, datetime):
                            raise TypeError(f"Unexpected type for selected_start_date: {type(selected_start_date)}")

                        if isinstance(selected_end_date, str):
                            selected_end_date = parser.parse(selected_end_date)
                        elif not isinstance(selected_end_date, datetime):
                            raise TypeError(f"Unexpected type for selected_end_date: {type(selected_end_date)}")

                        ks_date_domain = [
                            (rec.ks_date_filter_field.name, ">=", fields.Datetime.to_string(selected_start_date)),
                            (rec.ks_date_filter_field.name, "<=", fields.Datetime.to_string(selected_end_date))
                        ]
                    ...
                    if rec.ks_year_period and rec.ks_year_period != 0 and rec.ks_dashboard_item_type:
                        ...
                        for p in range(1, abs_year_period + 1):
                            ...
                            ks_date_domain.extend(['&', (date_field_name, ">=", fields.datetime.strftime(
                                selected_start_date - relativedelta(years=p) * sign_yp,
                                DEFAULT_SERVER_DATETIME_FORMAT)),
                                                   (date_field_name, "<=", fields.datetime.strftime(
                                                       selected_end_date - relativedelta(years=p)
    ...
    def _ks_get_chart_data(self, domain=[]):
        ...
        if rec.ks_dashboard_item_type and rec.ks_dashboard_item_type != 'ks_tile' and \
            ...
            if ((rec.ks_chart_data_count_type != "count" and ks_chart_measure_field) or (
                ...
                if (rec.ks_chart_groupby_type == 'date_type' and rec.ks_chart_date_groupby) or \
                    ...
                    if rec.ks_chart_groupby_type == 'date_type' and rec.ks_goal_enable and rec.ks_dashboard_item_type in [
                        ...
                        if isinstance(selected_start_date, datetime) and isinstance(selected_end_date, datetime):
                ...
                if len(ks_chart_measure_field) != 0 or rec.ks_chart_data_count_type == 'count':
                        ...
                        if ks_chart_groupby_relation_fields[0] in res:
                            if rec.ks_chart_groupby_type == 'date_type':
                                ...
                                if rec.ks_chart_date_groupby == "day" \
                                    ...
                                    if isinstance(res[ks_chart_groupby_relation_fields[0]], str):
                                        ...
                                        label = str(res[ks_chart_groupby_relation_fields[0]])
                            ...
                            if rec.ks_chart_data_count_type != 'count':
                                ...
                                if rec.ks_chart_measure_field_2 and rec.ks_dashboard_item_type == 'ks_bar_chart':
                                    for ress in rec.ks_chart_measure_field_2:
                                        if rec.ks_chart_sub_groupby_type == 'date_type':
                                            ...
                                            selection = res[ks_chart_groupby_relation_fields[1]] if res[ks_chart_groupby_relation_fields[1]] else ""
                                            if selection:
                                                labels_2.append(dict(self.env[rec.ks_model_name].fields_get(allfields=[ks_chart_groupby_relation_fields[1]])
                                                                 [ks_chart_groupby_relation_fields[1]]['selection'])[
                                ...
                                if rec.ks_chart_sub_groupby_type == 'date_type':
                                    ...
                                    if res[ks_chart_groupby_relation_fields[1]] is not False:
                                        field_value = res.get(ks_chart_groupby_relation_fields[1])
                                        if isinstance(field_value, tuple):
                                            labels.append(field_value[1])
                                        else:
                                            labels.append(str(field_value))
                                    else:
                                        labels.append(str(res[ks_chart_groupby_relation_fields[1]]))
                                elif rec.ks_chart_sub_groupby_type == 'other':
                    ...
                    ks_chart_data = {
                        ...
                        'ks_show_records': rec.ks_show_records, 'ks_data_format': rec.ks_data_format,
                        'ks_chart_item_color': rec.ks_chart_item_color,
                        'ks_bar_chart_stacked': rec.ks_bar_chart_stacked,
                        'ks_semi_circle_chart': rec.ks_semi_circle_chart, 'ks_hide_legend': rec.ks_hide_legend,
            ...
            return json.dumps(ks_chart_data, default=str)
        else:
            return False

    def get_item_config(self, params = {}):
        """
        :rtype : object
        :param : item object
        ...
        try:
            ...
            if ks_precision_digits < 0 or ks_precision_digits > 100:
                ks_precision_digits = 2
        except Exception:
            ks_precision_digits = 2
        ...
        ks_action_name = []
        if self.ks_action_lines:
            for res in self.ks_action_lines:
                my_dict = {}
                my_dict['id'] = res.id
        ...
        if self.ks_actions:
            context = {}
            try:
                context = safe_eval(self.ks_actions.context)
            except Exception:
                context = {}

            # Managing those views that have the access rights
            ks_actions = self.ks_actions.sudo()
            action['name'] = ks_actions.name
            action['type'] = ks_actions.type
            action['res_model'] = ks_actions.res_model
            action['views'] = ks_actions['views']
            action['view_mode'] = ks_actions.view_mode.replace('tree', 'list', 1)
            action['search_view_id'] = [ks_actions.search_view_id.id, 'search']
        ...
        if self.ks_unit and self.ks_unit_selection == 'monetary':
            try:
        ...
        common_config = {
            'id': self.id,
            'name': self.name if self.name else self.ks_model_id.name if self.ks_model_id else "Name",
            'ks_model_id': self.ks_model_id.id,
            'ks_model_name': self.ks_model_name,
            'ks_dashboard_item_type': self.ks_dashboard_item_type,
            'ks_company': self.ks_company_id.name if self.ks_company_id else False,
            'ks_ai_analysis': self.ks_ai_analysis or '',
            'item_data_source': self.data_source,
            'ks_info': self.ks_info,
            'ks_domain': self.ks_convert_into_proper_domain(self.ks_domain, self, item_domain1),
            'ks_dashboard_id': self.ks_dashboard_ninja_board_id.id,
            'ks_dashboard_name': self.ks_dashboard_ninja_board_id.name,
            'ks_model_display_name': self.ks_model_id.name,
            'ks_show_records': self.ks_show_records,
            'ks_data_calculation_type': self.ks_data_calculation_type,
            'ks_dashboard_manager': self.env.user.has_group('ks_dashboard_ninja.ks_dashboard_ninja_group_manager'),
            'action': action if action else False,
            'ks_precision_digits': ks_precision_digits,
            'zooming_enabled': self.env['ir.config_parameter'].sudo().get_param('ks_dashboard_ninja.enable_chart_zoom'),
            'ks_chart_cumulative': self.ks_chart_cumulative,
        }
        ...
        item_config = {}
        if self.ks_dashboard_item_type == 'ks_tile':
            item_config = { ** self._kpi_tile_common_config(item_domain1), ** self._get_tile_item_config(item_domain1) }
        elif self.ks_dashboard_item_type == 'ks_kpi':
            item_config = { ** self._kpi_tile_common_config(item_domain1), **self._get_kpi_item_config(item_domain1, item_domain2) }
        elif self.ks_dashboard_item_type == 'ks_to_do':
            item_config = { **self._get_todo_item_config() }
        elif self.ks_dashboard_item_type == 'ks_list_view':
            item_config = { **self._get_list_item_config(item_domain1) }
        else:
            item_config = { **self._get_graph_item_config(item_domain1) }

        return { **common_config, **item_config }

    def _get_kpi_item_config(self, item_domain1, item_domain2):
        self.ensure_one()
        return {
            # 'ks_kpi_data': self._ksGetKpiData(item_domain1, item_domain2),
            'ks_record_count_2': self._ksGetRecordCount_2(item_domain2),
            'ks_record_count': self._ksGetRecordCount(item_domain1),
            'ks_kpi_previous_period_data': self.ks_get_previous_period_data(),
            'ks_date_filter_selection': self.ks_date_filter_selection,
            'ks_previous_period': self.ks_previous_period,
            'ks_model_name_2': self.ks_model_name_2,
            'ks_data_comparison': self.ks_data_comparison,
            'ks_goal_enable': self.ks_goal_enable,
            'ks_data_calculation_type': self.ks_data_calculation_type,
            'ks_target_view': self.ks_target_view,
            'ks_standard_goal_value': self.ks_standard_goal_value,
        }

    def _kpi_tile_common_config(self, item_domain1):
        self.ensure_one()
        return {
            'ks_background_color': self.ks_background_color,
            'ks_font_color': self.ks_font_color,
            'ks_button_color': self.ks_button_color,
            'ks_default_icon_color': self.ks_default_icon_color,
            'ks_data_format': self.ks_data_format,
            'ks_icon_select': self.ks_icon_select,
            'ks_default_icon': self.ks_default_icon,
            'ks_unit': self.ks_unit,
            'ks_unit_selection': self.ks_unit_selection,
            'ks_chart_unit': self.ks_chart_unit,
            'ks_currency_id': self.ks_currency_id.id,
            'ks_multiplier': self.ks_multiplier,
            'ks_multiplier_active': self.ks_multiplier_active,
        }

    def _get_tile_item_config(self, item_domain1):
        self.ensure_one()
        return {
            'ks_layout': self.ks_layout,
            'ks_record_count': self._ksGetRecordCount(item_domain1),
        }

    def _get_todo_item_config(self):
        self.ensure_one()
        return {
            'ks_to_do_data': self.get_todo_data(),
            'ks_header_bg_color': self.ks_header_bg_color,
            'ks_font_color': self.ks_font_color,
            'ks_button_color': self.ks_button_color,
        }

    def _get_list_item_config(self, item_domain1):
        self.ensure_one()
        return {
            'ks_list_view_data': self._ksGetListViewData(item_domain1),
            'ks_list_view_type': self.ks_list_view_type,
            'ks_pagination_limit': self.ks_pagination_limit,
            'list_view_type': self.ks_list_view_type,
            'chart_actions_length': len(self.ks_action_lines) if self.ks_action_lines else False,
        }

    def _get_graph_item_config(self, item_domain1):
        self.ensure_one()
        ks_chart_data = self._ks_get_chart_data(item_domain1)
        return {
            'ks_pagination_limit': self.ks_pagination_limit, # in case of drill down list view
            'ks_chart_data': json.loads(ks_chart_data) if ks_chart_data else False,
            'chart_actions_length': len(self.ks_action_lines) if self.ks_action_lines else False,
            'ks_chart_item_color': self.ks_chart_item_color,
            'ks_data_calculation_type': self.ks_data_calculation_type,
            'ks_show_data_value': self.ks_show_data_value,
            'ks_dashboard_item_type': self.ks_dashboard_item_type,
            'ks_show_records': self.ks_show_records,
            'ks_data_format': self.ks_data_format,
            'ks_bar_chart_stacked': self.ks_bar_chart_stacked,
            'ks_semi_circle_chart': self.ks_semi_circle_chart,
            'ks_hide_legend': self.ks_hide_legend,
            'ks_data_label_type': self.ks_data_label_type,
            'ks_chart_measure_field_2': self.ks_chart_measure_field_2.ids,
            'ks_chart_cumulative_field': self.ks_chart_cumulative_field.ids,
        }
    ...
    def _ksGetListViewData(self, domain=[]):
        ...
        if rec.ks_chart_relation_groupby and not rec.ks_list_view_group_fields:
            default_measure = self.env['ir.model.fields'].search([
                ('model_id', '=', self.ks_model_id.id),
                ('name', '!=', 'id'),
                ('name', '!=', 'sequence'),
                ('store', '=', True),
        ...
        if rec.ks_list_view_type and rec.ks_dashboard_item_type and rec.ks_dashboard_item_type == 'ks_list_view' \
            ...
            if not (ks_list_view_data and len(ks_list_view_data) > 0):
                ks_list_view_data = {
    ...
    def get_list_view_record(self, orderid, sort_order, ks_chart_domain, ksoffset=0,
        ...
        self.ensure_one()
        ks_list_view_data = {'label': [], 'fields': [], 'fields_type': [], 'store': [], 'fields_technical_name': [],
                             'records': [], 'columns': {},
                             'total': self.env[self.ks_model_name].search_count(ks_chart_domain)}
        ...
        if ks_limit:
            ...
            if ks_limit and ks_limit < self.ks_pagination_limit:
                ...
                limit = self.ks_pagination_limit
        if self._context.get('export_all', False):
            limit = ks_limit
            offset = 0
        ...
        if self.ks_list_view_type == "ungrouped":
            if self.ks_list_view_fields:
                ks_list_view_data = self.ks_fetch_list_view_data(self, ks_chart_domain, offset=ksoffset, initial_count=initial_count)
        elif self.ks_list_view_type == "grouped" and self.ks_list_view_group_fields \
                and self.ks_chart_relation_groupby:
            ks_list_fields = []
            column_fields_list = ['name', 'ttype', 'store', 'field_description']
            ...
            if self.ks_chart_groupby_type == 'relational_type':
                ...
                ks_list_view_data['columns'][self.ks_chart_relation_groupby.name] =\
                    self.ks_chart_relation_groupby.read(column_fields_list)[0]
                ...
                for res in self.ks_list_view_group_fields:
                    ...
                    ks_list_view_data['columns'][res.name] = res.read(column_fields_list)[0]
                    if not isinstance(ks_list_view_data['columns'][res.name]['id'], int):
                        del ks_list_view_data['columns'][res.name]['id']

                try:
                    ks_list_view_records = self.env[self.ks_model_name]. \
                    web_read_group(ks_chart_domain, ks_list_fields, [self.ks_chart_relation_groupby.name],
                               orderby=orderby, offset=ksoffset, lazy=False, limit=limit)
                except Exception as e:
                    ks_list_view_records = {}

                ks_list_view_data['total'] = ks_list_view_records.get('length', 0)
                ks_list_view_records = ks_list_view_records.get('groups', [])
                # ks_list_view_records = ks_list_view_records[:limit]
                ...
                for res in ks_list_view_records:
                    if all(list_fields in res for list_fields in ks_list_fields) \
                        ...
                        # counter = 0
                        data_row = {'groupById': res[self.ks_chart_relation_groupby.name] and res[self.ks_chart_relation_groupby.name][0], 'data': {},
                                    'domain': json.dumps(res['__domain']), 'ks_column_type': []}
                        for field_rec in ks_list_fields:
                            if isinstance(res[field_rec], tuple):
                                data_row['data'][field_rec] = {'tech_value': res[field_rec][0], 'value': res[field_rec][1]}
                            else:
                                data_row['data'][field_rec] = {'value': res[field_rec]}
                        ks_list_view_data['records'].append(data_row)
                ...
                ks_list_view_data['columns'][self.ks_chart_relation_groupby.name + ':' + ks_chart_date_groupby] =\
                    self.ks_chart_relation_groupby.read(['field_description', 'ttype', 'store'])[0]
                if not isinstance(ks_list_view_data['columns'][self.ks_chart_relation_groupby.name + ':' + ks_chart_date_groupby].get('id', False), int):
                    del ks_list_view_data['columns'][self.ks_chart_relation_groupby.name]['id']
                ks_list_view_data['columns'][self.ks_chart_relation_groupby.name + ':' + ks_chart_date_groupby]['field_description'] = \
                ...
                for res in self.ks_list_view_group_fields:
                    ...
                    ks_list_view_data['columns'][res.name] = res.read(['field_description', 'ttype', 'store'])[0]
                    if not isinstance(ks_list_view_data['columns'][res.name].get('id', False), int):
                        del ks_list_view_data['columns'][res.name]['id']
                ...
                if self.ks_goal_enable and self.ks_list_target_deviation_field:
                    ...
                    if self.ks_list_target_deviation_field.name in ks_list_field:
                        ...
                        del ks_list_view_data['columns'][self.ks_list_target_deviation_field.name]
                try:
                    ks_list_view_records = self.env[self.ks_model_name].web_read_group(ks_chart_domain, ks_list_field + list_target_deviation_field,
                ...
                for res in ks_list_view_records:
                    if all(list_fields in res for list_fields in ks_list_fields):
                        ...
                        if res[ks_selection_field]:
                            data_row['data'][ks_selection_field] = {'value': dict(
                                self.env[self.ks_model_name].fields_get(allfields=ks_selection_field)
                                [ks_selection_field]['selection'])[res[ks_selection_field]],
                                                'tech_value': res[ks_selection_field]}
                        else:
                            data_row['data'][ks_selection_field] = { 'value': '', 'tech_value': False }
                        for field_rec in ks_list_fields:
                            data_row['data'][field_rec] = {'value': res[field_rec]}
                        ks_list_view_data['records'].append(data_row)
        ...
        if self.ks_multiplier_active and self.ks_list_view_type == 'grouped':
            for ks_multiplier in self.ks_multiplier_lines:
                column = ks_multiplier.ks_multiplier_fields.name
                if column in ks_list_view_data['columns']:
                    for record in ks_list_view_data['records']:
                        if record['data'][column]['value']:
                            record['data'][column]['value'] *= ks_multiplier.ks_multiplier_value
    ...
    def ks_fetch_list_view_data(self, rec, ks_chart_domain, limit=15, offset=0, ks_export_all=False, initial_count=0):
        ks_list_view_data = {'label': [], 'fields': [], 'fields_type': [], 'columns': {}, 'store': [],
                             'type': 'ungrouped', 'records': [], 'model': self.ks_model_name,
                             'total': self.env[self.ks_model_id.model].search_count(ks_chart_domain)}
        ...
        if self._context.get('export_all', False):
            limit = ks_limit
            offset = 0
        self.ks_list_view_fields = self.ks_list_view_fields.exists()
        if self.ks_list_view_fields:
            ks_list_view_data['list_view_type'] = 'other'
            ...
            for res in self.ks_list_view_fields:
                ks_list_view_data['columns'][res.name] = res.read(['name', 'ttype', 'store', 'field_description'])[0]
                if not isinstance(ks_list_view_data['columns'][res.name]['id'], int):
                    del ks_list_view_data['columns'][res.name]['id']
        ...
        for res in ks_list_view_records:
            ...
            for field_rec in ks_list_view_fields:
                if isinstance(res[field_rec], datetime):
                    data_row['data'][field_rec] = {
                        'value': fields.Datetime.context_timestamp(self, res[field_rec]).strftime("%D %T")
                    }
                elif isinstance(res[field_rec], date):
                    data_row['data'][field_rec] = {
                        'value': fields.Datetime.context_timestamp(self, fields.Datetime.to_datetime(
                            res[field_rec])).strftime("%D")
                    }
                elif ks_list_view_field_type[counter] == "many2one":
                    data_row['data'][field_rec] = {'value': res[field_rec][1] if res[field_rec] else res[field_rec], 'tech_value': res[field_rec][0] if res[field_rec] else res[field_rec]}
                elif ks_list_view_field_type[counter] == "selection" and res.get(field_rec, False):
                    data_row['data'][field_rec] = {'value': dict(
                        self.env[self.ks_model_name].fields_get(allfields=[field_rec])
                        [field_rec]['selection'])[res[field_rec]], 'tech_value': res[field_rec]}
                else:
                    data_row['data'][field_rec] = {'value': res[field_rec]}
                counter += 1
            ks_list_view_data['records'].append(data_row)
    ...
    def ks_get_previous_period_data(self):
        switcher = {
            'l_day': 'ls_day',
            't_week': 'ls_week',
            't_month': 'ls_month',
        ...
        ks_date_data = False
        if self.ks_date_filter_selection == "l_none":
            date_filter_selection = self.ks_dashboard_ninja_board_id.ks_date_filter_selection
        else:
            date_filter_selection = self.ks_date_filter_selection
        ...
        if (ks_date_data):
            ...
            proper_domain = self.ks_get_previous_period_domain(self.ks_domain, previous_period_start_date,
                                                              previous_period_end_date, self.ks_date_filter_field)
            ks_record_count = 0.0

            if self.ks_record_count_type == 'count':
                ks_record_count = 0
                try:
                    ks_record_count = self.env[self.ks_model_name].search_count(proper_domain)
                except Exception as E:
                    ks_record_count = 0
                return ks_record_count

            elif self.ks_record_field:
                try:
                    data = \
                        self.env[self.ks_model_name].read_group(proper_domain, [self.ks_record_field.name], [], lazy=False)[0]
                except Exception as E:
                    data = {}
                if self.ks_record_count_type == 'sum':
                    return data.get(self.ks_record_field.name, 0) if data.get('__count', False) and (
                        data.get(self.ks_record_field.name)) else 0
                else:
                    return data.get(self.ks_record_field.name, 0) / data.get('__count', 1) \
                        if data.get('__count', False) and (data.get(self.ks_record_field.name)) else 0
            else:
                return False
        else:
            return False
            ...
            if all(measure_field in res for measure_field in ks_chart_measure_field):
                ...
                if ks_chart_measure_field:
                    ...
                    if res.get('__count'):
                        ...
                        count = 0
                    data = count
                    if 'datasets' not in ks_chart_data:
                        ks_chart_data['datasets'] = []

                    if not ks_chart_data['datasets']:
    ...
    # FIXME: why self is initialized under record ??
    def ks_fetch_drill_down_data(self, domain, sequence):
        ...
        if action_line.ks_chart_type == 'ks_list_view':
            ks_list_view_data = {'label': [], 'type': 'grouped', 'records': [], 'columns': {},
                                 'fields_type': [], 'total': self.env[self.ks_model_id.model].search_count(domain),
            ...
            if action_line.ks_item_action_field.ttype == 'many2one':
                ks_list_view_data['columns'][action_line.ks_item_action_field.name] = \
                    action_line.ks_item_action_field.read(column_fields_list)[0]
                if not isinstance(ks_list_view_data['columns'][action_line.ks_item_action_field.name]['id'], int):
                    del ks_list_view_data['columns'][action_line.ks_item_action_field.name]['id']
                ...
                ks_list_fields.append(action_line.ks_item_action_field.name)
                ks_list_view_data['columns'][action_line.ks_item_action_field.name + ':' + action_line.ks_item_action_date_groupby] = \
                    action_line.ks_item_action_field.read(['field_description', 'ttype', 'store'])[0]
                if not isinstance(ks_list_view_data['columns'][action_line.ks_item_action_field.name + ':' + action_line.ks_item_action_date_groupby]['id'], int):
                    del ks_list_view_data['columns'][action_line.ks_item_action_field.name + ':' + action_line.ks_item_action_date_groupby]['id']
                ks_list_view_data['columns'][action_line.ks_item_action_field.name + ':' + action_line.ks_item_action_date_groupby]['field_description'] = \
                    action_line.ks_item_action_field.field_description + ' : ' + action_line.ks_item_action_date_groupby
            ...
            if record.ks_multiplier_active:
                for ks_multiplier in record.ks_multiplier_lines:
                    column = ks_multiplier.ks_multiplier_fields.name
                    if column in ks_list_view_data['columns']:
                        # index = ks_list_view_data['columns'][label].index(label)
                        for i in range(0, len(ks_list_view_data['records'])):
                            ks_list_view_data['records'][i]['data'][column]['value'] *= ks_multiplier.ks_multiplier_value
            return {"ks_list_view_data": ks_list_view_data, "ks_list_view_type": "grouped",
                    'next_sequence': sequence + 1, 'ks_action_name': "".join(ks_action_name.split(" "))}
        else:
            ks_chart_data['ks_dashboard_item_type'] = ks_chart_type
            ks_chart_measure_field = []
            ks_chart_measure_field_with_type = []
            ks_chart_measure_field_ids = []
            ks_chart_measure_field_2 = []
    ...
    # TODO: Fix the workflow for the functionality: very inappropriate
    def ks_get_next_offset(self, offset, item_domain={}):
        # record = self.browse(ks_item_id)
        ks_offset = offset['offset']
        item_domain = item_domain.get('ks_domain_1', [])
        ks_list_domain = self.ks_convert_into_proper_domain(self.ks_domain, self, item_domain)
        if self.ks_list_view_type == 'grouped':
            orderby = self.ks_sort_by_field.id
            sort_order = self.ks_sort_by_order
            ks_list_view_data = self.get_list_view_record(orderby, sort_order, ks_list_domain, ksoffset=ks_offset)

        else:
            # TODO : Dont know why this record is attached to arguments , patching is done based on the requirement - It should be fixed
            ks_list_view_data = self.ks_fetch_list_view_data(self, ks_list_domain, offset=ks_offset)
...
class KsDashboardItemsActions(models.Model):
    ...
    ks_sort_by_field = fields.Many2one('ir.model.fields',
                    domain = "[('model_id', '=', ks_model_id), ('name', '!=', 'id'), ('name', '!=', 'sequence'), ('store', '=', True), "
                                 "('ttype', 'not in', ['one2many', 'binary', 'char', 'text', 'boolean', 'html'])]",
