...
class BoraVendorBillApprovalRequestWizard(models.TransientModel):
    _name = 'bora.vendor.bill.approval.request.wizard'
    _description = 'Bora Vendor Bill Approval Request Wizard'
    ...
    def default_get(self, fields_list):
        ...
        move_id = self.env.context.get('default_bora_move_id') or res.get('bora_move_id')
        if is_update and move_id:
            move = self.env['account.move'].browse(move_id)
            if move.bora_bill_pm1_approved and move.bora_bill_pm1_id:
                res['bora_pm1_already_approved'] = True
                res['bora_approver1_user'] = move.bora_bill_pm1_id.id
        return res

    @api.depends('bora_move_id')
    def _compute_show_approver2(self):
        for wiz in self:
            if wiz.bora_move_id._has_bill_approval_config():
                wiz.bora_show_approver2 = (
                    wiz.bora_move_id._get_bill_approval_config().is_dual_approval()
                )
            else:
    ...
    def _compute_approver_user_ids(self):
        for wiz in self:
            if wiz.bora_move_id._has_bill_approval_config():
                config = wiz.bora_move_id._get_bill_approval_config()
                wiz.bora_approver1_user_ids = config.bora_bill_pm1_ids
                wiz.bora_approver2_user_ids = config.bora_bill_pm2_ids
            else:
    ...
    @api.depends('bora_move_id', 'bora_approver1_user', 'bora_approver2_user', 'bora_is_update_mode')
    def _compute_approval_info(self):
        for wiz in self:
            if not wiz.bora_move_id._has_bill_approval_config():
                wiz.bora_approval_info = '<p>No vendor bill approval configuration found.</p>'
                continue
            config = wiz.bora_move_id._get_bill_approval_config()
            move = wiz.bora_move_id
            html = ''

            if wiz.bora_is_update_mode and move.bora_bill_pm1_approved and move.bora_bill_pm1_id:
                html += (
                    ...
                    '</div>'
                ) % move.bora_bill_pm1_id.name

            html += '<div class="alert alert-info">'
            html += '<h5><strong>Vendor Bill Approval Request</strong></h5>'
            html += '<p>You are about to submit this vendor bill for approval before confirmation.</p>'
    ...
    def action_confirm_request(self):
        ...
        is_update = self.env.context.get('bora_is_update', False)
        move = self.bora_move_id

        if is_update and move.bora_bill_pm1_approved and move.bora_bill_pm1_id:
            if move.bora_bill_pm1_id != self.bora_approver1_user:
                raise UserError(_(
        ...
        preserve_pm1 = (
            is_update
            and move.bora_bill_pm1_id
            and move.bora_bill_pm1_id == self.bora_approver1_user
            and move.bora_bill_pm1_approved
        )

        if is_update:
            move._bora_cancel_bill_activities()
            write_vals = {
            ...
            pre_state = move.bora_bill_pre_approval_state or 'draft'
            move.write({'state': pre_state})

        move.bora_do_request_bill_approval(
        ...
        if preserve_pm1:
            # bora_do_request_bill_approval reset pm1_approved — restore it
            move.write({'bora_bill_pm1_approved': True})
            # Remove the fresh PM1 activity (PM1 already approved)
            self.env['mail.activity'].sudo().search([
                ('res_model', '=', 'account.move'),
                ('res_id', '=', move.id),
                ('user_id', '=', move.bora_bill_pm1_id.id),
                ('summary', 'ilike', 'Bill Approval'),
            ]).unlink()
            # Create PM2 activity now (mirrors what happens when PM1 approves)
            if move.bora_bill_pm2_id:
                move._create_bill_approval_activity(move.bora_bill_pm2_id, 'Confirm')

        return {'type': 'ir.actions.act_window_close'}

    def action_cancel(self):
        """If user closes the wizard without selecting approvers, reset the bill back to draft."""
        move = self.bora_move_id
        if move and move.state == 'bill_approval_pending' and not move.bora_bill_pm1_id:
            pre_state = move.bora_bill_pre_approval_state or 'draft'
            move.write({
                'state': pre_state,
