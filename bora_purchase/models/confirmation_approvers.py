...
class POConfirmationApproverTeam(models.Model):
    _name = 'purchase.order.confirmation.approvers'
    _description = 'Confirmation Approvers'
    _rec_name = 'user_id'
    _order = 'sequence'
    ...
    def _compute_existing_user_ids(self):
        ...
        existing_user_ids_list = self.env['purchase.order.confirmation.approvers'].search([]).user_id.ids
    ...
    def unlink(self):
        ...
        for approver in self:
            ...
            pos = self.env['purchase.order'].search([
                ('approval_users_ids_for_confirmation.user_id', '=', approver.user_id.id)
            ])
            affected_pos |= pos  # accumulate affected POs

            # Delete linked PO approval users with empty state
            for po in pos:
                records_to_delete = po.approval_users_ids_for_confirmation.filtered(
        ...
        all_approvers = self.env['purchase.order.confirmation.approvers'].search([], order='sequence')
        ...
        for po in affected_pos:
            po_users = po.approval_users_ids_for_confirmation.sorted(key='sequence')
