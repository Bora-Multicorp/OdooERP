...
class VendorApprovalConfig(models.Model):
    ...
    name = fields.Char(
        ...
        default='Global Vendor Approval Configuration',
        required=True,
    ...
    ks_approval_mode = fields.Selection([
       ...
       help='Single Level: Only Approver 1 needs to approve.\n'
            'Two Level: Both Approver 1 and Approver 2 must approve before vendor is approved.')
    
    # Approver Users - Multiple users support
    ks_approver_1_ids = fields.Many2many(
        'res.users',
        'ks_vendor_approval_approver_1_rel',
        'config_id',
        'user_id',
        string='Approvers 1',
        required=True,
        help='First/Primary approvers for vendor approval requests (select one when requesting approval)',
    )
    ks_approver_2_ids = fields.Many2many(
        'res.users',
        'ks_vendor_approval_approver_2_rel',
        'config_id',
        'user_id',
        string='Approvers 2',
        help='Second approvers for vendor approval requests (select one when requesting approval, required for two level approval mode)',
    )
    ...
    def get_approvers_by_level(self, approver_level):
        ...
        """
        self.ensure_one()
        if approver_level == 'approver_1' or approver_level == 'approver1':
            return self.ks_approver_1_ids
        elif approver_level == 'approver_2' or approver_level == 'approver2':
    ...
    def get_approver1_users(self):
        """Get all users configured as Approver 1 (backward compatibility)"""
        config = self.get_config()
        if config:
            return config.ks_approver_1_ids
        return self.env['res.users']

    @api.model
    def get_approver2_users(self):
        """Get all users configured as Approver 2 (backward compatibility)"""
        config = self.get_config()
        if config:
            return config.ks_approver_2_ids
        return self.env['res.users']

    @api.model
    def get_all_approval_users(self):
        """Get all users assigned to any approval level (backward compatibility)"""
        config = self.get_config()
        if config:
            return config.get_all_approvers()
        return self.env['res.users']
