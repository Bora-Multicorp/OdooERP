...
class InheritSurveyInvite(models.TransientModel):
    ...
    send_by_sms = fields.Boolean("Send By SMS", tracking=True)
    sms_recipient = fields.Many2one("res.partner", "SMS Recipients", tracking=True)
    mobile_no = fields.Char("Mobile Number", tracking=True)
    ...
    def send_sms(self):
        ...
        if len(resp['error']) != 0:
            raise ValidationError(
                _("Invalid Number %s. They should start with 0 or 61 followed by 9 digits") % self.mobile_no)
        else:
