...
class KsCancelApprovalRequestWizard(models.TransientModel):
    _name = 'ks.sale.cancel.approval.request.wizard'
    _description = 'KS Sale Cancel Approval Request Wizard'
    ...
    def _compute_approver_user_ids(self):
        ...
        for wizard in self:
            ...
            config = order._get_approval_config()
            wizard.ks_approver1_user_ids = config.ks_cancel_pm1_ids
            wizard.ks_approver2_user_ids = config.ks_cancel_pm2_ids
    ...
    @api.depends('ks_sale_order_id', 'ks_approver1_user', 'ks_approver2_user')
    def _compute_approval_info(self):
        ...
        for wizard in self:
            ...
            info_html += '<p>You are about to send a cancellation request for this Sale Order.</p>'
    ...
    def action_confirm_request(self):
        ...
        # Open reason wizard (passes ks_is_update so it can be forwarded if needed)
        return {
