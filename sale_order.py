...
class SaleOrder(models.Model):
    ...
    def _ks_do_approve_confirmation_with_reason(self, reason, current_user):
        ...
        if is_pm1_approving and config.is_dual_approval() and self.ks_confirm_pm2_id:
            ...
            note = self._get_approval_activity_note('confirm', self.name, self.ks_confirm_request_user_id.name if self.ks_confirm_request_user_id else _('Unknown'))
        ...
        if approval_complete:
            ...
            if self.ks_confirm_pm1_id:
                self._mark_activity_done(self.ks_confirm_pm1_id, feedback=_("Approval completed - Sale Order confirmed"))
            if self.ks_confirm_pm2_id:
                self._mark_activity_done(self.ks_confirm_pm2_id, feedback=_("Approval completed - Sale Order confirmed"))
    ...
    def _ks_cancel_approval_request(self):
        ...
        if self.ks_confirm_pm1_id:
            self._mark_activity_done(self.ks_confirm_pm1_id, feedback=_("Approval request cancelled by %s") % self.env.user.name)
        if self.ks_confirm_pm2_id:
            self._mark_activity_done(self.ks_confirm_pm2_id, feedback=_("Approval request cancelled by %s") % self.env.user.name)
    ...
    def _ks_do_approve_cancel_with_reason(self, reason, current_user):
        ...
        if is_pm1_approving and config.is_dual_approval() and self.ks_cancel_pm2_id:
            ...
            note = self._get_approval_activity_note('cancel', self.name, self.ks_cancel_request_user_id.name if self.ks_cancel_request_user_id else _('Unknown'), self.ks_cancel_request_reason)
    ...
    def _ks_do_approve_edit_with_reason(self, reason, current_user):
        ...
        if is_pm1_approving and config.is_dual_approval() and self.ks_edit_pm2_id:
            ...
            note = self._get_approval_activity_note('edit', self.name, self.ks_edit_request_user_id.name if self.ks_edit_request_user_id else _('Unknown'), self.ks_edit_request_reason)
        ...
        if approval_complete:
            # Mark all remaining activities as done (approval complete)
            if self.ks_edit_pm1_id:
                self._mark_activity_done(self.ks_edit_pm1_id, feedback=_("Approval completed - Edit permission granted"))
            if self.ks_edit_pm2_id:
                self._mark_activity_done(self.ks_edit_pm2_id, feedback=_("Approval completed - Edit permission granted"))
