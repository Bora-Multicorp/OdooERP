...
class PurchaseOrder(models.Model):
    """Extend purchase.order to add exchanged amount calculation based on exchange rate"""
    ...
    def _compute_exchanged_amount(self):
        ...
        Example: If PO total is 200 USD and rate is 50, then exchanged_amount = 200 / 50 = 4
        """
        for order in self:
...
class PurchaseOrderLine(models.Model):
    """Extend purchase.order.line to add exchanged currency amount calculation based on exchange rate"""
    ...
    @api.depends('price_subtotal', 'price_unit', 'product_qty', 'discount', 'order_id.rate', 'order_id.currency_id', 'order_id.company_currency_id', 'order_id.is_exchange')
    def _compute_exchanged_amount(self):
        ...
        for line in self:
            ...
            if not line_total or line_total == 0:
                ...
                line_total = (line.price_unit or 0.0) * (line.product_qty or 0.0) * discount_factor
