...
class KsPurchaseReceiptApprovalRequestWizard(models.TransientModel):
    _name = 'ks.purchase.receipt.approval.request.wizard'
    _description = 'KS Purchase Receipt Approval Request Wizard'
    ...
    def default_get(self, fields_list):
        ...
        if is_update and picking_id:
            ...
            if picking.ks_receipt_pm1_approved and picking.ks_receipt_pm1_id:
                res['ks_pm1_already_approved'] = True
                res['ks_approver1_user'] = picking.ks_receipt_pm1_id.id
        return res

    @api.depends('ks_picking_id')
    def _compute_show_approver2(self):
        for wiz in self:
            if wiz.ks_picking_id._has_receipt_approval_config():
                wiz.ks_show_approver2 = (
                    wiz.ks_picking_id._get_receipt_approval_config().is_dual_approval()
                )
            else:
    ...
    def _compute_approver_user_ids(self):
        for wiz in self:
            if wiz.ks_picking_id._has_receipt_approval_config():
                config = wiz.ks_picking_id._get_receipt_approval_config()
                wiz.ks_approver1_user_ids = config.ks_receipt_pm1_ids
                wiz.ks_approver2_user_ids = config.ks_receipt_pm2_ids
            else:
                wiz.ks_approver1_user_ids = False
                wiz.ks_approver2_user_ids = False

    @api.depends('ks_approver1_user_ids', 'ks_approver2_user_ids',
                 'ks_approver1_user', 'ks_approver2_user')
    ...
    def _compute_approval_info(self):
        for wiz in self:
            if not wiz.ks_picking_id._has_receipt_approval_config():
                wiz.ks_approval_info = '<p>No purchase receipt approval configuration found.</p>'
                continue
            config = wiz.ks_picking_id._get_receipt_approval_config()
            picking = wiz.ks_picking_id
            html = ''

            if wiz.ks_is_update_mode and picking.ks_receipt_pm1_approved and picking.ks_receipt_pm1_id:
                html += (
                    ...
                    '</div>'
                ) % picking.ks_receipt_pm1_id.name

            html += '<div class="alert alert-info">'
            html += '<h5><strong>Purchase Receipt Approval Request</strong></h5>'
            html += '<p>You are about to submit this receipt for approval before validation.</p>'
    ...
    def action_confirm_request(self):
        ...
        if is_update and picking.ks_receipt_pm1_approved and picking.ks_receipt_pm1_id:
            if picking.ks_receipt_pm1_id != self.ks_approver1_user:
                raise UserError(_(
        ...
        preserve_pm1 = (
            is_update
            and picking.ks_receipt_pm1_id
            and picking.ks_receipt_pm1_id == self.ks_approver1_user
            and picking.ks_receipt_pm1_approved
        )

        if is_update:
            # PM1's activity is already marked done when PM1 approves; only PM2's will be active
            picking._ks_cancel_receipt_activities()
            write_vals = {
            ...
            pre_state = picking.ks_receipt_pre_approval_state or 'assigned'
            picking.write({'state': pre_state})

        picking.ks_do_request_receipt_approval(
        ...
        if preserve_pm1:
            # ks_do_request_receipt_approval reset pm1_approved — restore it
            picking.write({'ks_receipt_pm1_approved': True})
            # Remove the fresh PM1 activity (PM1 already approved)
            self.env['mail.activity'].sudo().search([
                ('res_model', '=', 'stock.picking'),
                ('res_id', '=', picking.id),
                ('user_id', '=', picking.ks_receipt_pm1_id.id),
                ('summary', 'ilike', 'Receipt Approval'),
            ]).unlink()
            # Create PM2 activity now (mirrors what happens when PM1 approves)
            if picking.ks_receipt_pm2_id:
                picking._create_receipt_approval_activity(picking.ks_receipt_pm2_id, 'Validate')
    ...
    def action_cancel(self):
        ...
        picking = self.ks_picking_id
        if picking and picking.state == 'receipt_approval_pending' and not picking.ks_receipt_pm1_id:
            pre_state = picking.ks_receipt_pre_approval_state or 'assigned'
            picking.write({
                'state': pre_state,
