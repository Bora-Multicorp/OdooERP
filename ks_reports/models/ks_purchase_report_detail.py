...
class KsPurchaseReportDetail(models.Model):
    ...
    def _get_xlsx_headers(self):
        """Column headers for XLSX export (match list view order)."""
        return [
            'PO Number', 'Bill or Advance', 'Invoice No', 'Invoice Date', 'Purchase Manager',
            'INWARD IN', 'Vendor Name', 'Vendor GSTIN', 'Model', 'PO Qty', 'GRN Qty', 'Difference',
            'Rate', 'Amount', 'Remarks', 'Invoice', 'E - Invoices', 'E-way Bill', 'IMEI/ Serial No',
            'DOCKET', 'EWAYBILL NO', 'DOCKET No', 'TRANSPORTER', 'Vehicle No.', 'Remarks (PO)',
        ]

    def _get_xlsx_row(self, r):
        """One row of data for XLSX (same order as headers)."""
        return [
            r.purchase_id.name if r.purchase_id else '',
            r.payment_term_id.name if r.payment_term_id else '',
            r.invoice_id.name if r.invoice_id else '',
            r.invoice_date.strftime('%Y-%m-%d') if r.invoice_date else '',
            r.user_id.name if r.user_id else '',
            r.company_id.name if r.company_id else '',
            r.partner_id.name if r.partner_id else '',
            r.vendor_gstin or '',
            r.product_id.display_name if r.product_id else '',
            r.po_qty,
            r.grn_qty,
            r.difference,
            r.rate,
    ...
    def generate_xlsx_report(self):
        """
        Generate XLSX report for current recordset (or all if empty).
        ...
        records = self
        if not records:
            records = self.search([], order=self._order)
        ...
        sheet = workbook.add_worksheet('Purchase Report Detail')
        headers = self._get_xlsx_headers()
        for col, header in enumerate(headers):
            sheet.write(0, col, header, header_fmt)

        row = 1
        number_cols = (9, 10, 11, 12, 13)  # po_qty, grn_qty, difference, rate, amt
        for r in records:
            values = self._get_xlsx_row(r)
            for col, val in enumerate(values):
                # If value is empty, show N/A
                if col in number_cols:
                    is_empty = val is None or (isinstance(val, (int, float)) and val == 0)
                else:
                    is_empty = val is None or (isinstance(val, str) and not (val or '').strip())
                display_val = 'N/A' if is_empty else val
                if col in number_cols:
                    if isinstance(display_val, (int, float)):
                        sheet.write_number(row, col, display_val, number_fmt)
                    else:
                        sheet.write(row, col, display_val, data_fmt)
                else:
                    sheet.write(row, col, display_val, data_fmt)
            row += 1

        for col in range(len(headers)):
            sheet.set_column(col, col, min(22, max(12, len(str(headers[col])) + 2)))
